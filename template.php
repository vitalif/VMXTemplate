 ...
 *   change               : Tuesday,  Sep 25, 2007
 *   copyright            : (C) 2001 The phpBB Group
 *   copyright            : (C) 2006-2007 VitaliF
...
/**
 * Original Template class by Nathan Codding of the phpBB group.
 ...
 * <!--# Comment #-->
 * <!-- IF block_name.block_name.etc.[...].varname --> [[...]] <!-- END --> = parse [[...]] only if varref {block_name.block_name.etc.[...].varname} is set and is true (if PHP thinks that it's true :-))
 * <!-- IF! block_name.block_name.etc.[...].varname --> [[...]] <!-- END --> = parse [[...]] only if varref {block_name.block_name.etc.[...].varname} is not set or is not true
 *
 * Документация:
 * <!-- BEGIN * --> и <!-- END * --> могут быть даже на одной строке
 ...
 * {и_так_далее.#} = номер текущей итерации блока "блок1.блок2.и_так_далее", если "и_так_далее" - самый внутренний по вложенности блок
 ...
 *
 *  $template = new Template ("/directory/with/templates");
 *  $template->cachedir = "/directory/for/caching/compiled/code"; # your script must have write access to this directory / выполняемый скрипт должен иметь доступ на запись в эту директорию
 *  $template->no_pparse = true; # highly recommended with previous versions, but unnecessary with this / важно с предыдущими версиями, но необязательно с этой
 *
 *  $template->set_filenames(array('body' => 'hello.tpl'));
 *  $template->assign_vars(array(
 *      "HELLO" => "Hello, world!",
 *      "SHOW1" => 1,
 *      "TEXT1" => "I'll be visible (SHOW1 = true)",
 *      "TEXT2" => "And I not (SHOW2)",
 *  ));
 ...
 *  ));
 *
 *  $template->pparse('body'); # same as "echo $template->rparse('body');" if $template->no_pparse is true / то же, что и "echo $template->rparse('body');", если $template->no_pparse истинно
 *
 * Template example/пример шаблона:
 *
 *  <html>
 *  <head>
 *    <title>Hello, world! by template.php</title>
 *  </head>
 *  <body>
 *    <font size="+3">{HELLO}</font><br />
 *    <!-- IF SHOW1 -->{TEXT1}<!-- END --><br />
 *    <!-- IF SHOW2 -->{TEXT2}<!-- END --><!-- IF! SHOW2 -->SHOW2 is false<!-- END --><br />
 *    <!-- BEGIN language -->
 *      {language.NAME} --> <a href="{language.URL}">{language.URL}</a><br />
 *    <!-- END language -->
 *  </body>
 *  </html>
 */
...
class Template
{
    ...
    function compile($code, $do_not_echo = false, $retvar = '')
    {
        ...
        $code = preg_replace ('/^\s*(<!-- (?:BEGIN|END|IF!?|ELSE|SET|ENDSET) .*?-->)\s*$/m', "\x01\\1\x01\n", $code);
        $ncode = $code;
        do {
            $code = $ncode;
            $ncode = preg_replace ("/([^\x01])(<!-- (?:BEGIN|END|IF!?|ELSE|SET|ENDSET) .*?-->)/m", "\\1\x01\\2", $code);
        } while ($ncode != $code);
        $ncode = $code;
        do {
            $code = $ncode;
            $ncode = preg_replace ("/<!-- (?:BEGIN|END|IF!?|ELSE|SET|ENDSET) .*?-->(?=[^\x01])/m", "\\0\x01", $code);
        } while ($ncode != $code);
        ...
        for ($i = 0; $i < $line_count; $i++)
        {
            ...
            if (preg_match('#^<!-- BEGIN (!?)([A-Za-z0-9\-_]+?) ([A-Za-z \t\-_0-9]*)-->$#', $code_lines[$i], $m)) // ((APPEND|PREPEND) (([A-Za-z0-9\-_]+?\.)*)([A-Za-z0-9\-_]+?) )?
            {
                ...
                $block_nesting_level++;
                $block_names[$block_nesting_level] = $b_bn;
                ...
                if (preg_match ('#^[ \t]*AT ([0-9]+)[ \t]*(?:([0-9]+)[ \t]*)?$#', $b_mod, $nem))
                {
                    $cbstart = isset ($nem[1]) ? (0+$nem[1]) : 0;
                    $cbcount = (isset ($nem[2]) ? (0+$nem[2]) + $cbstart : 0);
                }
                if (preg_match ('#^[ \t]*MOD ([1-9][0-9]*) ([0-9]+)[ \t]*$#', $b_mod, $nem))
                {
                    $cbstart = $nem[2];
                    $cbplus = '+='.$nem[1];
                }
                ...
                if ($block_nesting_level < 2)
                {
                    // Block is not nested.
                    if (!$cbcount)
                        $code_lines[$i] = '$_' . $b_bn . '_count = ( isset($this->_tpldata[\'' . $b_bn . '.\']) ) ? sizeof($this->_tpldata[\'' . $b_bn . '.\']) : ' . $addmodes[$block_nesting_level][2] . ';';
                    else $code_lines[$i] = '$_' . $b_bn . '_count = min (@sizeof($this->_tpldata[\'' . $b_bn . '.\']), ' . $cbcount . ');';
                }
                else
                {
                    ...
                    if (!$cbcount)
                        $code_lines[$i] = '$_' . $b_bn . '_count = ( isset(' . $varref . ') ) ? sizeof(' . $varref . ') : ' . $addmodes[$block_nesting_level][2] . ';';
                    else $code_lines[$i] = '$_' . $b_bn . '_count = min (@sizeof(' . $varref . '), ' . $cbcount . ');';
                }
                if (!$b_not)
                    $code_lines[$i] .= "\n" . 'for ($_' . $b_bn . '_i = ' . $cbstart . '; $_' . $b_bn . '_i < $_' . $b_bn . '_count; $_' . $b_bn . '_i' . $cbplus . ')';
                else
                    $code_lines[$i] .= "\n" . 'for ($_' . $b_bn . '_i = ' . $cbcount . '; $_' . $b_bn . '_i < ' . max($cbcount, 1) . '; $_' . $b_bn . '_i' . $cbplus . ')';
                $code_lines[$i] .= "\n" . '{';
            }
            else if (preg_match('#<!-- END\s+(.*?)-->#', $code_lines[$i], $m))
            {
            ...
            else if (preg_match('#<!-- IF(!?)\s+((?:[a-zA-Z0-9\-_]+\.)+)?([a-zA-Z0-9\-_/]+) -->#is', $code_lines[$i], $m))
            {
                $varref = $this->generate_block_data_ref (substr ($m[2], 0, strlen($m[2])-1), true) . '[\'' . $m[3] . '\']';
                $code_lines[$i] = 'if('.(empty($m[1]) ? '!' : '').'empty('.$varref.')) {';
            }
            else if (preg_match('#<!-- ELSE -->#is', $code_lines[$i], $m))
                $code_lines[$i] = '} else {';
            else if (!$in_set && preg_match('#^\s*<!--\s*SET\s+((?:[a-zA-Z0-9\-_]+\.)+)?([a-zA-Z0-9\-_/]+)\s*-->\s*$#s', $code_lines[$i], $m))
            {
                $varref = $this->generate_block_data_ref (substr ($m[1], 0, strlen($m[1])-1), true) . '[\'' . $m[2] . '\']';
                $code_lines[$i] = "\$${retvar}_tmp = \$$retvar;\n\$$retvar = '';\n$varref = eval(<<<ENDSET";
                $in_set = true;
            }
            else if ($in_set && preg_match('/^\s*<!--\s*ENDSET\s*-->\s*$/s', $code_lines[$i], $m))
            {
                $code_lines[$i] = "return \\\$_str;\nENDSET\n);\n\$$retvar = \$${retvar}_tmp;";
                $in_set = false;
            }
            ...
            if ($in_set && !preg_match('/<<<ENDSET$/', $code_lines[$i]))
